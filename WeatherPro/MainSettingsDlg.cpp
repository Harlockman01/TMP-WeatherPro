// MainSettingsDlg.cpp: archivo de implementación
//
...
// MainSettingsDlg message handler
...
BOOL MainSettingsDlg::OnInitDialog()
{
        ...
        ctrl_combobox_data_source.AddString(cmn::GetStringRes(IDS_API_OM));
...
void MainSettingsDlg::OnBnClickedButtonOpenApiSettings()
{
        ...
        if (gui_api_type == ApiType::WeatherComCnSpider) {
                ...
                dlg.DoModal();
        } else if (gui_api_type == ApiType::OpenMeteo) {
                OptionsOmDlg dlg(this);
                dlg.DoModal();
        }
}
